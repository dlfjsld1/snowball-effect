...
func _ready() -> void:
	...
	for profile in range(4):
		...
		var visual_field := frame.get_field_visual_rect()
		...
		assert(is_equal_approx(field.size.y, 768.0))
		assert(is_equal_approx(field.get_center().x, 800.0))
		assert(visual_field.position == Vector2(EXPECTED_FIELD_X[profile], 50.0))
		assert(visual_field.size == Vector2(EXPECTED_FIELD_WIDTHS[profile], 800.0))
		assert(field.end.y == 818.0 and visual_field.end.y == 850.0)
		...
		assert(frame.get_field_bezel_rect().end.y == 868.0)
		...
		assert(frame.get_cashout_line_y() == 818.0)
	...
	print("S5-G4 frame kit verification passed: 4 profiles, Paper-8 frame, logical/visual Cashout strip bounds")
	get_tree().quit()
