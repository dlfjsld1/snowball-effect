...
func _ready() -> void:
	...
	_expect(hud.stage_score_label.text == "STAGE 1", "HUD must display stage score once.")
	_expect(hud.run_score_label.text == "RUN 1", "HUD must display run score once.")
	_expect(ledger.stage_score == 1.0 and ledger.run_score == 1.0, "HUD subscription must not duplicate gameplay score.")

	hud.reset_view()
	_expect(hud.stage_score_label.text == "STAGE 0" and hud.ball_count_label.text == "BALLS 0", "HUD reset must clear display state.")
