...
func _ready() -> void:
	...
	var initial_pause: Dictionary = _audio_manager.get_debug_snapshot()
	_expect(_music_key() == &"bgm_pause", "Pause immediately after Start must replace Ground BGM.")
	_expect(initial_pause["paused_stage_music_key"] == &"bgm_ground", "Immediate Pause must preserve Ground BGM.")
	resume_requested.emit()
	_expect(_music_key() == &"bgm_ground", "Resume must restore Ground BGM after an immediate Pause.")
	...
	stage_state_changed.emit(&"READY")
	_expect(_music_key() == &"bgm_title", "Main-menu READY state must replace Result BGM with Title BGM.")
	_expect(_audio_manager.get_debug_snapshot()["music_playing"], "Title BGM must be actively playing after Result-to-Main transition.")
