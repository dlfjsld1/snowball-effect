...
HUD의 기본 정보 계약은 Stage 이름, 점수, 시간, 현재 아이템, 일시정지, 공 족보다. 최고 공과 Ball Count는 debug/result 정보로 남긴다.

공 족보는 현재 Stage의 local 공 5종을 작은 픽셀 아이콘으로 낮은 단계부터 높은 단계까지 **세로 한 열**로 배열한다. Stage 진입 시 첫 공만 표시하고 새 공을 처음 만들 때 다음 아이콘과 이름을 공개한다. 5칸 housing은 고정하되 미발견 공의 정답은 출력하지 않는다. 별도의 `NEXT` Spawn 예고 영역은 두지 않으며 Play Field를 가리지 않도록 기계 프레임에 배치한다.
