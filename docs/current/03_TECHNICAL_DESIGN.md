...
- `Paddle/Visual` Sprite2D는 대쉬 상승/복귀 동안 squash·stretch와 밝기 펄스를 적용한다. `DashGhostNear`·`DashGhostMid`·`DashGhostFar` Sprite2D는 이동 방향 반대편 28·56·84 logical units에 패들 형태의 희미한 초록색 잔상을 세 단계로 남긴다. 잔상 Sprite의 기울기는 패들을 따르지만 위치는 Paddle local 축이 아니라 화면/world의 대쉬 방향 기준이므로, 90°·180° 회전 상태에서도 상승 대쉬의 잔상은 항상 화면 아래에 남는다. 잔상은 대쉬 종료 뒤 최대 `0.55s` 동안 fade-out해 순간 돌진을 명확히 보인다. 쐐기형 speed trail은 사용하지 않는다. 본체와 Sprite 잔상은 같은 PNG를 사용하며, 본체 `_draw()` 렌더링은 추가하지 않는다.
...
논리 인력과 모든 일반 Snowball 흡수는 BallSimulationManager에서 적용한다. Black Hole 자체는 일반 Merge와 하단 Cashout scan에서 제외하고, Play Field 하단을 전용 반사 경계로 사용한다. Black Hole은 흡수로 radius·mass·force를 키우지 않는다. Black Hole 이동도 동일한 Paddle provider의 continuous collision query를 사용하므로, Mouse sweep·회전 contact velocity·impact cap과 separation correction을 일반 Snowball과 동일하게 적용한다.
...
흡수 contact가 확정되면 일반 Cashout commit 전에 해당 공을 한 번 소비한다. StageRuntime은 첫 Black Hole phase 시작 시 Run Score baseline을 한 번 snapshot하고, matching Phase 완료 뒤 실제 `PLAYING` delta만 흡수 피해 clock에 누적한다. 첫 `1.0s`는 공만 흡수하고 점수 피해는 적용하지 않는다. 이후 공별 후보 `min(calculate_cashout_score(ball) × 0.125, phase_entry_run_score × 0.25)`를 계산하되, 같은 `0.5s` damage window에서 실제 적용하는 stage/run 차감 합계는 `phase_entry_run_score × 0.10`으로 제한한다. 같은 tick의 다중 공과 두 Black Hole source도 이 shared window budget을 사용한다. baseline은 이후 Cashout이나 흡수로 갱신하지 않는다. `stage_score`는 0에서 clamp하고, 지속 손실로 `run_score <= 0`이면 `run_score = 0`으로 고정한 뒤 즉시 failure lock과 Run End를 요청한다. Time Bonus, Cashout 전용 popup, Merge, Settlement 중복 반영은 없다.
