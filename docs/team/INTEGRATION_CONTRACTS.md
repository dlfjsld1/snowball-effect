...
| Content ItemManager | `item_planet_spawned(item_type: StringName, world_position, radius)` | Content S7-G2 optional-item visual | 모든 item type이 공유하는 승인된 Rescue Beacon Capsule Item Ball의 최초 표시용 read-only 신호. spawn/score/simulation을 변경하지 않는다. |
...
현재 S7-G1C producer는 마지막 유효 hit commit에서 `item_planet_broken`을 먼저, `item_orb_spawned`를 바로 다음에 발행한다. Presentation의 승인된 Rescue Beacon Capsule rupture break는 별도 Orb와 잠시 겹칠 수 있으며 Orb의 이동·획득·miss를 지연시키지 않는다. 파괴 전 Item Ball만 공용 Rescue Beacon Capsule이면 되고 Orb는 파괴 확정 뒤 첫 item-specific visual이다.
