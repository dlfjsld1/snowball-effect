...
### S8-G3 Title·Result·Retry UI

- Owner: Content/Systems
- Owned Files: `scripts/ui/title_screen.gd`, `scripts/ui/result_panel.gd`, `scenes/ui/title_screen.tscn`, `scenes/ui/result_panel.tscn`, `tests/content/**`
- Integration Point: `start_requested`, `retry_requested`; read-only result snapshot 표시 API.
- Dependencies: S8-G2 result snapshot schema와 S0-G2 UI mount 계약.
- Verification: score/highest stage/highest ball 표시; Start/Retry 요청 한 번; UI가 runtime state를 직접 초기화하지 않음.
...
- Integration Point: Core result snapshot, Content Result UI의 `retry_requested`, 모든 Owner의 reset API 연결.
- Dependencies: S8-G2, S8-G3, Presentation reset API.
- Verification: run score/highest stage/highest ball 표시; `CLEARED→Result`; Retry가 배열·점수·타이머·settlement/shift/item/presentation lock 완전 초기화.
