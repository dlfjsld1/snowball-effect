...
| S4-G3 Web release·1,000공 stretch 스트레스 | Core | VERIFIED | 2026-08-12 현재 공 크기와 예상 동시 활성 규모에 맞춰 필수 Gate를 실제 Web 500개 최저 30 FPS 이상으로 재설정하고, 1,000개는 stretch/torture 병목 기록으로 유지했다. reusable stress scene에서 100/500/1,000공의 FPS·physics ms·candidate·merge·max active·memory/bucket growth를 측정했다. clean Web actual browser의 500공 Merge OFF는 평균 `52.1`/최저 `38.6 FPS`, physics `2.18ms`로 필수 Gate를 통과했고 console warning/error 0이었다. Desktop 1,000공은 Merge OFF 평균 `58.7`/최저 `55.0`, Merge ON 평균 `35.8`/최저 `34.0 FPS`; Web 1,000공은 OFF 평균 `27.4`/최저 `20.9`, ON 평균 `19.9`/최저 `15.3 FPS`로 stretch 병목을 기록했다. OFF physics `4.39ms` 대비 전체 frame 비용이 커 `BallRenderer` 공별 `draw_circle` 경로를 첫 최적화 후보로 남긴다. Q-S4와 S4 Exit Gate 통과. |
