...
- S5-G2/G3는 `VERIFIED`다. S5-G3는 Presentation 병목을 피하기 위해 Main의 임시 자동 완료 adapter로 상태 중재를 연결했다. S5-G4가 완료되면 adapter를 제거하고 같은 완료 API에 Presentation signal을 연결한다. 다음 병렬 작업은 Presentation S5-G4다.
