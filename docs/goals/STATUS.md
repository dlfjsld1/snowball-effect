...
- S3-G1은 최신 5종·비연속 Stage chain으로 재정렬·재검증되어 `VERIFIED`다. Lv7 Red Giant와 Lv9 Nebula는 catalog에는 남지만, 팀 합의에 따라 기본 Run에는 의도적으로 포함하지 않는다. S3-G2와 S3-G3은 Stage data 소비·점수/time source-of-truth·tick 종료 중재까지 `VERIFIED`다. 다음 Core Goal은 S3-G4 Snapshot Settlement다.
