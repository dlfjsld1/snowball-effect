...
- S3-G1은 최신 5종·비연속 Stage chain으로 재정렬·재검증되어 `VERIFIED`다. Lv7 Red Giant와 Lv9 Nebula는 catalog에는 남지만, 팀 합의에 따라 기본 Run에는 의도적으로 포함하지 않는다. S3-G2~G4는 Stage data 소비·점수/time source-of-truth·tick 종료 중재·idempotent Settlement까지 `VERIFIED`다. 다음은 Integration S3-G5 Clear·Fail 상태 통합이며, Presentation S3-G6도 병렬 시작 가능하다.
