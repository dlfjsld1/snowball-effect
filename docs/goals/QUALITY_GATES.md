...
| Q-S6 | burst에서도 패들·공 가독성, FX budget 작동, 중요한 이벤트 우선; FIRST_CONTACT visible 전 pause 수락, 6종 layer 조립, normal/reduced 구성 합계 정확히 `2.00s`·실제 관찰 `2.00s ±0.05s`, `(run_epoch,event_id)` matching finish, duplicate/stale/reset, 일반 resume와 Black Hole S8 handoff 분리 |
...
| Q-S8 | 첫 Lv14→이동 Black Hole 전환, matching FIRST_CONTACT 완료 전 Phase/`phase_id` 0회·완료 뒤 기존 S8 Phase 1회, L2→L3 Frame/Play Field 동기 확장 후 gameplay 재개; stable Merge candidate order에서 moving entity 최대 2와 same-tick slot reservation, capacity overflow `Lv13+Lv13` source pair 보존·non-Merge 반사/분리, generic Lv14·extra discovery/Cashout/인력/흡수 경로 0; 모든 일반 Snowball contact 흡수의 Cashout `12.5%`/phase-entry Run Score `25%` 상한 패널티·단일 흡수 즉사 방지·반복 손실의 run score 0 Game Over·`480/1200` per-source pull·`1500` 합산 cap·`450` Black Hole 상호 척력·하단 반사·비성장 안정성; 이동 cue의 300-unit dashed ring·orbiting square dot 0, 승인 Void Cathedral C gold/violet 근거리 arc·짧은 이동 trail·generic circle fallback 0; 충분한 상대속도의 두 번째 Black Hole 접촉 시 1회 finale→UI 제거→타이틀/Clear Score/Main Menu, Retry/Main 완전 초기화, Desktop/Web 수락 |
...
4. **S6-G2 entry:** 위 두 Goal이 `STATUS.md`에서 실제 Evidence와 함께 `VERIFIED`된 뒤에만 Presentation controller 구현을 시작한다. S6-G2 자체는 Native layer/normal·reduced 설정 합계 `2.00s`와 관찰 허용오차 `±0.05s`/reset capture, 실제 Web Tween·active Play Field clip/dim·input focus·console·resume/Black Hole handoff를 추가로 검증한다.
