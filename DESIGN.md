...
이 파일은 Snowball Effect의 디자인 진입점이다. 상세 명세는 [docs/design/README.md](docs/design/README.md), 현재 채택한 목업과 제작 기준은 [docs/design/09_APPROVED_VISUAL_DIRECTION_V4.md](docs/design/09_APPROVED_VISUAL_DIRECTION_V4.md)를 따른다. 게임 규칙과 기술 계약이 충돌하면 저장소의 `AGENTS.md` 문서 우선순위를 적용한다.

## Product Context

Snowball Effect는 공을 합쳐 더 큰 공을 만들고, Stage를 Clear할 때마다 아케이드 기계와 실제 Play Field가 함께 확장되는 픽셀 아케이드 게임이다. 플레이어가 기억해야 할 장면은 중앙축을 유지한 캐비닛이 좌우로 열리며 더 큰 우주를 수용하는 순간이다.
...
- Play Field seed 폭은 `600 → 760 → 920 → 1080`이며 마지막 `1080`은 terminal L3/Final Result profile이다.
